...
fs::path cache_path_for_contract(const std::string& cache_dir,
    ...
    return cache_root(cache_dir, server_url) / (sanitize_component(contract) + ".csv");
}
...
HistoryCacheResult HistoryDataClient::ensure_contracts_cached(
    ...
    for (const auto& contract : contracts) {
        const fs::path path = cache_path_for_contract(cache_dir_, server_url_, contract);
        ...
        const std::string target = "/api/bars.csv?contract=" + url_encode(contract);
