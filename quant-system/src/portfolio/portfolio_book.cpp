...
PortfolioBook::PortfolioBook(double initial_cash)
    : cash_(initial_cash), realized_pnl_(0.0), unrealized_pnl_(0.0), equity_(initial_cash) {}
...
PortfolioSnapshot PortfolioBook::snapshot() const {
    ...
    // 负权益时设 cash_weight=NaN 标记破产状态
    snapshot.cash_weight = std::abs(equity_) <= kEpsilon ? 1.0
        : (equity_ < 0.0 ? std::nan("") : cash_ / equity_);
